...
def packet_callback(packet):
    ...
    if(RMSE < 1.001*threshold):
        sendp(packet, iface=args.out_interface)
    else: 
        #val=1 #potentially bad packet, drop
...
def main():
    ...
    sniff(prn=packet_callback, iface=args.in_interface, store=0)
    #sniff(offline=args.in_interface, prn=packet_callback, store=0)
