...
def add_routes(bridge, addr):
    ...
    snort_ports=[]
    snort_ports+=[find_of_port(bridge, SNORT_NAME, interface) for interface in interfaces]

    in_rte=[1]+snort_ports+[proxy_port]
    out_rte1=[proxy_port]+[2]
    out_rte2=[proxy_port]+[1]
    direct_rte=[1]+snort_ports+[2]

    # IP client to proxy & back
    for in_port,out_port in pairwise(in_rte):
        cmd=('/usr/bin/sudo /usr/bin/ovs-ofctl add-flow {} "priority=100 ip ' +
             'in_port={} nw_src={} nw_dst={} actions=output:{}"')
        cmd=cmd.format(bridge, in_port, CLIENT_IP, addr, out_port)        
        subprocess.check_call(shlex.split(cmd))
        cmd=('/usr/bin/sudo /usr/bin/ovs-ofctl add-flow {} "priority=100 arp ' +
             'in_port={} nw_src={} nw_dst={} actions=output:{}"')
        cmd=cmd.format(bridge, in_port, CLIENT_IP, addr, out_port)        
        subprocess.check_call(shlex.split(cmd))
    for in_port,out_port in pairwise(reversed(in_rte)):
        cmd=('/usr/bin/sudo /usr/bin/ovs-ofctl add-flow {} "priority=100 ip ' +
             'in_port={} nw_src={} nw_dst={} actions=output:{}"')
        cmd=cmd.format(bridge, in_port, addr, CLIENT_IP, out_port)        
        subprocess.check_call(shlex.split(cmd))
        cmd=('/usr/bin/sudo /usr/bin/ovs-ofctl add-flow {} "priority=100 arp ' +
             'in_port={} nw_src={} nw_dst={} actions=output:{}"')
        cmd=cmd.format(bridge, in_port, addr, CLIENT_IP, out_port)        
    ...
    # proxy to server
    for in_port,out_port in pairwise(out_rte1):
        cmd=('/usr/bin/sudo /usr/bin/ovs-ofctl add-flow {} "priority=100 ip ' +
             'in_port={} nw_src={} nw_dst={} actions=output:{}"')
        cmd=cmd.format(bridge, in_port, addr, SERVER_IP, out_port)
        subprocess.check_call(shlex.split(cmd))
        cmd=('/usr/bin/sudo /usr/bin/ovs-ofctl add-flow {} "priority=100 arp ' +
             'in_port={} nw_src={} nw_dst={} actions=output:{}"')
        cmd=cmd.format(bridge, in_port, addr, SERVER_IP, out_port)
        subprocess.check_call(shlex.split(cmd))
    for in_port,out_port in pairwise(reversed(out_rte1)):
        cmd=('/usr/bin/sudo /usr/bin/ovs-ofctl add-flow {} "priority=100 ip ' +
             'in_port={} nw_src={} nw_dst={} actions=output:{}"')
        cmd=cmd.format(bridge, in_port, SERVER_IP, addr, out_port)
        subprocess.check_call(shlex.split(cmd))
        cmd=('/usr/bin/sudo /usr/bin/ovs-ofctl add-flow {} "priority=100 arp ' +
             'in_port={} nw_src={} nw_dst={} actions=output:{}"')
        cmd=cmd.format(bridge, in_port, SERVER_IP, addr, out_port)
    ...
    # client to server
    for in_port,out_port in pairwise(direct_rte):
        cmd=('/usr/bin/sudo /usr/bin/ovs-ofctl add-flow {} "priority=100 ip ' +
             'in_port={} nw_src={} nw_dst={} actions=output:{}"')
        ...
        cmd=cmd.format(bridge, in_port, CLIENT_IP, SERVER_IP, out_port)
        subprocess.check_call(shlex.split(cmd))
    for in_port,out_port in pairwise(reversed(direct_rte)):
        cmd=('/usr/bin/sudo /usr/bin/ovs-ofctl add-flow {} "priority=100 ip ' +
             'in_port={} nw_src={} nw_dst={} actions=output:{}"')
...
def teardown(bridge):
    ...
    cmd='/usr/bin/sudo /usr/bin/ovs-docker del-ports {} {}'
    cmd=cmd.format(bridge, SNORT_NAME)
    subprocess.call(shlex.split(cmd))
