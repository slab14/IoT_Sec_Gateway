...
locatedPacket bohateiIPSProc(int ipsIndex, locatedPacket inPkt){
	...
	if (ipsDiffConnStates[ipsIndex][inPkt.packet.srcIP] == Count_0)
	{
		if (inPkt.packet.tcpSYN){
			printf("WOOT1\n");	
        		ipsDiffConnStates[ipsIndex][inPkt.packet.srcIP] = Count_1;
	...
	if (ipsDiffConnStates[ipsIndex][inPkt.packet.srcIP] == Count_1){
		if (inPkt.packet.tcpSYN){
			printf("WOOT2\n");	
        	ipsDiffConnStates[ipsIndex][inPkt.packet.srcIP] = Count_2;
